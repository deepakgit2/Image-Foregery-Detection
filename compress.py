...
def dequantise(C,Q):
  ...
  return C * repmat( Q, C.shape / Q.shape, C.shape / Q.shape )
...
image = image_data('tiny.jpg')
v,r,c=im2vec(image['coef_arrays'][0].transpose(), image['quant_tables'][0].shape)
#v,r,c = im2vec(a.transpose(),(3,3))
v=v.transpose()
print v,r,c

q_t = qtable.transpose() #alternate qtable(:)
n_v = np.array([ [i] for j in range(np.size(q_t,0)) for i in q_t[j] ])
print vec2im(v*repmat(n_v,1,np.size(v,1)),(3,3),0,r,c)
